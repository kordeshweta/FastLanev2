...
export class AdminComponent implements OnInit{
    ...
    downloadDetails(ele){
        ...
        const sent_data = {id: ele.id, category, name: ele.cName};
    ...
    showAllDownloads(){
        ...
        const sent_data = {id: null, category: null};
        const dialofRef = this.dialog.open(DownloadDetailsDialog, {
            width: '1046px',
            height: '516px',
            data: sent_data
            ...
            this.componentservices.getDownloadDetails(this.data.id, this.data.category).subscribe(res => {
                if (res.status == true){
                    ...
                    for (let i = 0; i < arr.length; i++)
                    {
                        ...
                        const obj = {email: arr[i].email, purpose: arr[i].purpose, ddate: arr[i].createDate, customer:
        arr[i].customerName, proj_id: arr[i].projectID, remarks: arr[i].remark};
            ...
            this.componentservices.getAllDownloads().subscribe(res => {
                if (res.status == true){
                    ...
                    {
                        // console.log(arr[i]["email"])
                        const obj = {email: arr[i].email, skill_set: arr[i].entity, cName: arr[i].cName, purpose: arr[i].purpose, ddate: arr[i].createDate, customer:
            arr[i].customerName, proj_id: arr[i].projectID, remarks: arr[i].remark};
                        arr2.push(obj);
                    }
                    this.dataSource = new MatTableDataSource(arr2);
